...
1. **Add More App Signatures**
   ```cpp
   // In types.cpp — just add more SNI patterns
   if (sni.find("twitch") != std::string::npos)
       return AppType::TWITCH;
   if (sni.find("reddit") != std::string::npos)
       return AppType::REDDIT;
   ```

2. **Add Bandwidth Throttling (instead of hard block)**
   ```cpp
   // Delay packets instead of dropping them
...
3. **Add a Live Statistics Dashboard**
   ```cpp
   // Dedicated stats thread — prints live counters every second
   void statsThread() {
       while (running) {
           clearScreen();
           printLiveStats();
...
4. **Add QUIC / HTTP3 Support**
   - QUIC runs over **UDP port 443** (not TCP)
   - The SNI is inside the QUIC Initial packet, encrypted with a well-known key
   - Requires a separate QUIC parser — a great next challenge!

5. **Add Persistent Rules via Config File**
   ```json
   {
     "block_apps": ["YouTube", "TikTok"],
     "block_ips": ["192.168.1.50"],
     "block_domains": ["ads.google.com"]
   }
   ```
   - Load rules from JSON/YAML on startup
   - Hot-reload without restarting the engine

6. **Add Bidirectional Flow Tracking**
...
This DPI engine demonstrates real-world systems programming concepts:

| Concept | What You Learn |
|---------|---------------|
| **Network Protocol Parsing** | How packets are structured layer by layer |
| **Deep Packet Inspection** | Reading inside TLS to extract SNI |
| **Flow Tracking** | Stateful connection management with hash maps |
| **Multi-threaded Architecture** | Thread pools with load balancing |
| **Producer-Consumer Pattern** | Bounded thread-safe queues with condition variables |
| **Consistent Hashing** | Routing same flows to same worker without locks |

> **Key Insight:** Even HTTPS traffic leaks the destination domain in the TLS Client Hello as **SNI in plaintext** — this is the fundamental technique that powers real-world network firewalls and parental control systems.

---

## Questions?

The code is well-commented and mirrors the flow described in this document exactly.

- **New to networking?** Start with [`main_working.cpp`](src/main_working.cpp) — simple, linear, easy to follow.
- **Ready for concurrency?** Move to [`dpi_mt.cpp`](src/dpi_mt.cpp) — full multi-threaded pipeline.
- **Want to experiment?** Run `python3 generate_test_pcap.py` to generate fresh test traffic.
...
> *"Networks don't lie — every packet tells a story. I just built the engine to read them."*

A passionate systems programmer with a deep curiosity for how data flows across networks.
This DPI engine is a fusion of low-level C++ systems programming, real-world networking concepts,
and multi-threaded design — built from scratch with zero external dependencies.

---

*© 2026 Priyal Kumar. Built with curiosity, caffeine, and a love for systems programming.* ☕�
